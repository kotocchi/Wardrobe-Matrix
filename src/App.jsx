...
export default function App() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('wardrobe_matrix_rollback');
    if (saved) {
      try {
  ...
  const saveStateToStorage = (updatedSaved, updatedWish) => {
    localStorage.setItem('wardrobe_matrix_rollback', JSON.stringify({ savedOutfits: updatedSaved || savedOutfits, wishlist: updatedWish || wishlist }));
  };
